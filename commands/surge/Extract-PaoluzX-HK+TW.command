...
}
{
  ...
  if (line ~ /^\[/) {
    ...
    name = proxy_name(line)
    if (is_hk_name(name) || is_tw_name(name)) {
      found = 1
    } else {
...
/usr/bin/printf '{"timestamp":"%s","target":"surge-profile","operation":"filter_proxy_region_hk_tw","setting_paths":["%s"],"old_value_summary":"full profile before HK+TW filtering","new_value_summary":"HK+TW profile, kept_proxies=%s","backup_path":"%s","rollback_instruction":"cp -p %s %s","verification":"section [Proxy] contains only HK/TW-tagged entries","confirmation_required":false,"confirmation_received":false}\n' \
