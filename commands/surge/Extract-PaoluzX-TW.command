...
/usr/bin/printf '{"timestamp":"%s","target":"surge-profile","operation":"filter_proxy_region_tw","setting_paths":["%s"],"old_value_summary":"full profile before TW-only filtering","new_value_summary":"TW-only profile, kept_proxies=%s","backup_path":"%s","rollback_instruction":"cp -p %s %s","verification":"section [Proxy] contains only TW-tagged entries","confirmation_required":false,"confirmation_received":false}\n' \
